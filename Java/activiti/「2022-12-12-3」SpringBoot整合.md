...
```java
package com.workplat.workflow.config;
...
[https://github.com/Activiti/Activiti/releases/tag/activiti-5.22.0](https://github.com/Activiti/Activiti/releases/tag/activiti-5.22.0)

![image-20221212150213798](https://picgo.kwcoder.club/202208/202212121502115.png)

解压后，找到`modules/activiti-webapp-explorer2/src/main/webapp`下的`modeler.html`、`favicon.ico`、`editor-app`、`diagram-viewer`四个目录或文件夹拷贝到项目的`resources/static`中。

![image-20221212151206144](https://picgo.kwcoder.club/202208/202212121512209.png)

项目中新建activiti包，拷贝`modules/activiti-modeler/src/main/java/org/activiti/rest/editor/main/StencilsetRestResource.java`、`model`下面的两个类，一起拷贝近activiti包中。

修改`editor-app/app-cfg.js`中的`contextRoot`，修改为自己web应用的上下文路径。

在`static`目录下新建`stencilset.json`文件，并修改`StencilsetRestResource`类中对应的路径。

填充内容为「点击大纲跳转至stencilset.json」，用于汉化。

在`static/editor-app/i18n`中新建`zh-CN.json`文件，内容为「点击大纲跳转至zh-CN.json」，用于国际化。

修改`editor-app`下`app.js`中的`$translateProvider.preferredLanguage('en');`为：
...
`ModelSaveRestResource.saveModel`参数声明中的`@RequestBody`更改为`@RequestParam`。

`ModelSaveRestResource`文件修改为：
 ...
 */
package com.workplat.workflow.activiti;
